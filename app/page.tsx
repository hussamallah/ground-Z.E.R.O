...
type QuestionCardProps = {
  ...
  total: number;                    // 21
  stem: string;
  options: Option[];                // 3..12 but you show 5 in Q1/Q2
...
function QuestionCard({
  qnum, total, stem, options, value, onSelect, onNext, disabled
}: QuestionCardProps) {

  // 1–9 number keys select options quickly
  useEffect(() => {
  ...
  return (
    <section
      ...
      className="mx-auto max-w-4xl rounded-3xl bg-slate-900/70 backdrop-blur
                 ring-1 ring-slate-700/60 shadow-2xl p-6 sm:p-8">
      {/* Header */}
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
        <p className="text-xs text-slate-400">
          Tip: press <kbd className="px-1 py-0.5 bg-slate-800/70 rounded">1–5</kbd> to choose. Press <kbd className="px-1 py-0.5 bg-slate-800/70 rounded">Enter</kbd> to continue.
        </p>
        <button
          onClick={onNext}
