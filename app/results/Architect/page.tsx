...
import ArchitectSecondaries from '../../components/GroundZero_Architect_13_Secondaries.json';
...
const CURRENT_ARCHETYPE = "Architect";
...
const SecondaryArchetypeSection = ({ finalWinner, secondaryFace, pureOneFace }: { finalWinner: Seed | null, secondaryFace?: Seed | null, pureOneFace?: boolean }) => {
    if (!finalWinner || finalWinner.face !== 'Architect') return null;
    const data: any = ArchitectSecondaries as any; // Using the imported JSON

    let content: any = null;
    if (pureOneFace) {
        content = data.baseline;
    } else if (secondaryFace && secondaryFace.face) {
        const title = `Architect × ${secondaryFace.face}`;
    ...
    <div className="mb-10 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {triad.map((item: any) => {
                ...
                const isPrizeActive = prizeActivation?.state === "ACTIVE" && item.family === "Truth";
                
                return (
                    <div
                        key={item.family}
                        className={`bg-white/5 rounded-lg p-4 flex flex-col cursor-pointer transition-all duration-200 hover:bg-white/10 ${
                            isPrizeActive ? 'bg-gradient-to-br from-purple-500/10 to-purple-600/5 border border-purple-500/20' : ''
                        }`}
