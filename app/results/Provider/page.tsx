...
import ProviderSecondaries from '../../components/GroundZero_Provider_13_Secondaries.json';
...
const CURRENT_ARCHETYPE = "Provider";
...
const SecondaryArchetypeSection = ({ finalWinner, secondaryFace, pureOneFace }: { finalWinner: Seed | null, secondaryFace?: Seed | null, pureOneFace?: boolean }) => {
    if (!finalWinner || finalWinner.face !== 'Provider') return null;
    const data: any = ProviderSecondaries as any;

    let content: any = null;
    if (pureOneFace) {
        content = data.baseline;
    } else if (secondaryFace && secondaryFace.face) {
        const title = `Provider × ${secondaryFace.face}`;
