...
import VisionarySecondaries from '../../components/GroundZero_Visionary_13_Secondaries.json';
    ...
    <div className="mb-6 pt-6">
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-y-4">
            ...
            <div className="flex items-center gap-x-3">
                ...
                <img
                    src={`/${(finalWinner?.face || 'Visionary').toLowerCase()}.png`}
                    alt={`${finalWinner?.face || 'Visionary'} emblem.`}
                    width={72}
                    height={72}
                    className="rounded"
                    style={{
...
const SecondaryArchetypeSection = ({ finalWinner, secondaryFace, pureOneFace }: { finalWinner: Seed | null, secondaryFace?: Seed | null, pureOneFace?: boolean }) => {
    if (!finalWinner || finalWinner.face !== 'Visionary') return null;
    const data: any = VisionarySecondaries as any;

    let content: any = null;
    if (pureOneFace) {
        content = data.baseline;
    } else if (secondaryFace && secondaryFace.face) {
        const title = `Visionary × ${secondaryFace.face}`;
