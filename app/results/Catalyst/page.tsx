...
import CatalystSecondaries from '../../components/GroundZero_Catalyst_13_Secondaries.json';
...
const CURRENT_ARCHETYPE = "Catalyst";
...
export default function CatalystResultsPage() {
    ...
    <div className="mb-6 pt-6">
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-y-4">
            ...
            <div className="flex items-center gap-x-3">
                ...
                <Image
                    src={`/${(finalWinner?.face || 'Catalyst').toLowerCase()}.png`}
                    alt={`${finalWinner?.face || 'Catalyst'} emblem.`}
                    width={72}
                    height={72}
                    className="rounded"
                    style={{
...
const SecondaryArchetypeSection = ({ finalWinner, secondaryFace, pureOneFace }: { finalWinner: Seed | null, secondaryFace?: Seed | null, pureOneFace?: boolean }) => {
    if (!finalWinner || finalWinner.face !== 'Catalyst') return null;
    const data: any = CatalystSecondaries as any;

    let content: any = null;
    if (pureOneFace) {
        content = data.baseline;
    } else if (secondaryFace && secondaryFace.face) {
        const title = `Catalyst × ${secondaryFace.face}`;
