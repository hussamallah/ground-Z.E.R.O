...
const SecondaryArchetypeSection = ({ finalWinner, secondaryFace, pureOneFace }: { finalWinner: Seed | null, secondaryFace?: Seed | null, pureOneFace?: boolean }) => {
    ...
    const data: any = PartnerSecondaries as any;

    let content: any = null;
    if (pureOneFace) {
        content = data.baseline;
    } else if (secondaryFace && secondaryFace.face) {
        const title = `Partner × ${secondaryFace.face}`;
                ...
                <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-white/80 font-medium">{A.toFixed(0)}%</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-white/60">Sense:</span>
                            <div className="w-16 h-1 bg-white/20 rounded-full">
                                <div 
                                    className="h-1 rounded-full transition-all duration-200"
                                    style={{
                            ...
                            <span className="text-white/80 font-medium">{S.toFixed(0)}%</span>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-white/80 font-medium">{R.toFixed(0)}%</span>
                        </div>
                    </div>
                    <div className="text-white/60">
                        {familyResults.length} families
                    </div>
                </div>
            </div>
        </div>
    ...
    <div className="mb-10 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {triad.map((item: any) => {
                ...
                const isPrizeActive = prizeActivation?.state === "ACTIVE" && item.family === "Bonding";
                
                return (
                    <div
                        key={item.family}
                        className={`bg-white/5 rounded-lg p-4 flex flex-col cursor-pointer transition-all duration-200 hover:bg-white/10 ${
                            isPrizeActive ? 'bg-gradient-to-br from-pink-500/10 to-pink-600/5 border border-pink-500/20' : ''
                        }`}
...
const ResultsScreen = ({ taps, finalWinner, duels, secondaryFace, pureOneFace, onRestart, router }: { taps: Tap[], finalWinner: Seed | null, duels: MatchLog[], secondaryFace?: Seed | null, pureOneFace?: boolean, onRestart: () => void, router: any }) => {
    ...
    const download = () => {
        const familiesManifest = familyResults.map((res, i) => {
            ...
            return {
                family: fam,
                scores: { A: scores.A, S: scores.S, R: scores.R },
                share: { A: scores.A / total, S: scores.S / total, R: scores.R / total },
                winner: res.winner,
        ...
        a.href = url;
        a.download = `ground-zero-results-${finalWinner?.face?.toLowerCase() || 'unknown'}.json`;
    ...
    const winnerArchetype = computeFinal.winnerArchetype;
