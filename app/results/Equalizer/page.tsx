...
import EqualizerSecondaries from '../../components/GroundZero_Equalizer_13_Secondaries.json';
...
export default function EqualizerResultsPage() {
    ...
    <div className="mb-6 pt-6">
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-y-4">
            ...
            <div className="flex items-center gap-x-3">
                ...
                <img
                    src={`/${(finalWinner?.face || 'Equalizer').toLowerCase()}.png`}
                    alt={`${finalWinner?.face || 'Equalizer'} emblem.`}
                    width={72}
                    height={72}
                    className="rounded"
                    style={{
...
const SecondaryArchetypeSection = ({ finalWinner, secondaryFace, pureOneFace }: { finalWinner: Seed | null, secondaryFace?: Seed | null, pureOneFace?: boolean }) => {
    if (!finalWinner || finalWinner.face !== 'Equalizer') return null;
    const data: any = EqualizerSecondaries as any;

    let content: any = null;
    if (pureOneFace) {
        content = data.baseline;
    } else if (secondaryFace && secondaryFace.face) {
        const title = `Equalizer × ${secondaryFace.face}`;
    ...
    <div className="mb-10 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {triad.map((item: any) => {
                ...
                const isPrizeActive = prizeActivation?.state === "ACTIVE" && item.family === "Boundary";
                
                return (
                    <div
                        key={item.family}
                        className={`bg-white/5 rounded-lg p-4 flex flex-col cursor-pointer transition-all duration-200 hover:bg-white/10 ${
                            isPrizeActive ? 'bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20' : ''
                        }`}
